            ...
            try {
                ...
                if (signResult.status === 'finish') {
                    if (driver.session.account.signers.length > 1) {
                        ...
                        if (driver.session.account.signers.find(signer =>
                        (signer.key === 'GA2T6GR7VXXXBETTERSAFETHANSORRYXXXPROTECTEDBYLOBSTRVAULT'))) {
                            apiURL = 'https://vault.lobstr.co/api/transactions/';
                            signerName = 'Lobstr Vault';
                        }
                        if (driver.session.account.signers.find(signer =>
                        (signer.key === 'GCVHEKSRASJBD6O2Z532LWH4N2ZLCBVDLLTLKSYCSMBLOYTNMEEGUARD'))) {
                            apiURL = 'https://stellarguard.me/api/transactions';
                            signerName = 'StellarGuard';
                        }
                        ...
                        driver.modal.handlers.activate(signerName);
